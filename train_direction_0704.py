...
from scene.freq_utils import compute_oriented_frequencies,compute_oriented_frequencies_reload
...
def training(dataset, opt, pipe, saving_iterations, checkpoint_iterations, checkpoint, final_iter, json_file):
    first_iter = 0
    ...
    for iteration in range(first_iter, final_iter + 1):
        ...
        loss, valid_count = 0.0, 0
        # # # 3. 处理每个3D点
        # for i in range(voxel_xyz.shape[0]):
        #     pt = voxel_xyz[i, :]
        #     pt = R @ pt + t  # 变换到相机坐标系
        #     if pt[2] <= 0: continue  # 在相机后方
        #     pt_2d = (K @ pt) / pt[2]
        #     x, y = pt_2d[:2].to(torch.int64)
        #     # voxel_dir
        #     dir = voxel_dir[i, :]
        #     dir = (R @ dir.T).T  # 变换到相机坐标系
        #     dir_2d = dir[:2]
        #     dir_2d = dir_2d / (torch.norm(dir_2d, p=2) + 1e-8)  # normaliz.
        #     if 0 <= x < w and 0 <= y < h:  # fall into which patch
        #         patch_x = min(x // patch_size, f_mag.shape[1] - 1)
        #         patch_y = min(y // patch_size, f_mag.shape[0] - 1)
        #         # f_2d->3d
        #         ## A) z
        #         # distance = pt[2] # z:=pt[2]
        #         ## B) (x,y,z)
        #         # distance = torch.norm(pt)  # sqrt(X²+Y²+Z²)
        #         # freq_mag = f_mag[patch_y, patch_x] / (distance ** 0.8)  # or remove **0.8
        #         freq_orient = f_orient[patch_y, patch_x]
        #
        #         angle_rad = freq_orient * (math.pi / 180.0)
        #         gt_dir_2d = torch.stack([torch.cos(angle_rad), torch.sin(angle_rad)]).cuda()
        #         gt_dir_2d = gt_dir_2d / (torch.norm(gt_dir_2d, p=2) + 1e-8)  # normaliz.
        #         # loss
        #         cos_sim = torch.nn.functional.cosine_similarity(dir_2d, gt_dir_2d, dim=0)
        #         loss_patch = 1 - torch.abs(cos_sim)  # .mean()
        #         loss += loss_patch
        #         valid_count += 1
        ...
        print(f"loss={loss}, visible_voxel={valid_count}")
        loss.backward()
    ...
    for viewpoint_cam in tqdm(viewpoint_stack):  # , desc="Processing cameras"
        ...
        h, w = viewpoint_cam.image_height, viewpoint_cam.image_width
        f_mag, f_orient = compute_oriented_frequencies(gt_image, patch_size)
        # gt_image_path = os.path.join(args.source_path, f"images_{dataset.resolution}",viewpoint_cam.image_name+'.JPG')
        # f_mag, f_orient = compute_oriented_frequencies_reload(gt_image_path, patch_size)
        ...
        #         # distance = pt[2] # z:=pt[2]
        #         ## B) (x,y,z)
        #         distance = np.linalg.norm(pt)  # sqrt(X²+Y²+Z²)
        #         freq_mag = f_mag[patch_y, patch_x] / (distance ** 0.8)  # or remove **0.8
        #         point_frequencies[i].append(freq_mag)
        #         visible_pt+=1
        # print(f'visible in this view={visible_pt}')
        ...
        for pt_id in range(voxel_xyz.shape[0]):
            ...
            if 0 <= x < w and 0 <= y < h:  # fall into which patch
                ...
                ## A) z
                # distance = pt_cam[2]
                # freq_mag = f_mag[patch_y, patch_x] / (pt_cam[2] ** 0.8) # z:=pt_cam[2]
                ## B) (x,y,z)
    ...
    # ----------------------------------------------------
    save_results_json(final_frequencies, json_file)
...
if __name__ == "__main__":
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.save_iterations, args.checkpoint_iterations,
             args.start_checkpoint, args.final_iter, json_file)
